...
import { BadgeCheck, Banknote, Clock3, MapPin, Star } from "lucide-react";
...
export function TaskCard({ task }: { task: TaskFeedItem }) {
  ...
  return <Card className="overflow-hidden transition hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-md">
    ...
    <div className="border-t border-stone-100 px-5 py-3 dark:border-stone-800"><Link href={`/users/${task.customer.id}`} className="flex min-h-11 items-center gap-3 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"><UserAvatar name={task.customer.displayName} avatarUrl={task.customer.avatarUrl} className="size-9" /><span className="min-w-0"><span className="flex items-center gap-1 truncate text-sm font-bold">{task.customer.displayName}{task.customer.telegramVerified && <BadgeCheck className="size-4 shrink-0 text-sky-600" />}</span><span className="flex items-center gap-1 text-xs text-stone-500"><Star className="size-3.5 fill-amber-400 text-amber-400" />{task.customer.ratingCount ? task.customer.ratingAverage.toFixed(1) : "Новый профиль"}</span></span><span className="ml-auto text-xs font-bold text-emerald-700">Профиль</span></Link></div>
  </Card>;
}
