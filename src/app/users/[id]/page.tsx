...
import { ArrowLeft, BadgeCheck, CalendarDays, CheckCircle2, ShieldCheck, Star } from "lucide-react";
import { z } from "zod";
...
export default async function PublicUserPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const user = await prisma.user.findFirst({
    ...
    select: { id: true, displayName: true, avatarUrl: true, bio: true, ratingAverage: true, ratingCount: true, completedTasks: true, telegramVerifiedAt: true, phoneVerifiedAt: true, createdAt: true, roles: true },
  });
  if (!user) notFound();
