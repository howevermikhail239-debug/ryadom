...
import { BadgeCheck, BriefcaseBusiness, MapPin, ShieldCheck, Star, UsersRound, WalletCards } from "lucide-react";
...
export default async function ProfilePage() {
  ...
  const [createdCount, performedCount, favoritePlaces] = await Promise.all([
  ...
  return <main className="mx-auto min-h-dvh max-w-2xl px-4 pb-32 pt-5 sm:px-8">
    <header className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider text-emerald-700 dark:text-emerald-400">Аккаунт</p><h1 className="text-3xl font-black tracking-tight">Профиль</h1></div><ProfileActions /></header>
    ...
    <section className="mt-4 rounded-3xl border border-stone-200 bg-white p-5 dark:border-stone-800 dark:bg-stone-900"><h2 className="flex items-center gap-2 font-black"><MapPin className="size-5 text-emerald-700" /> Сохранённые места</h2>{favoritePlaces.length ? <div className="mt-3 space-y-2">{favoritePlaces.map((place) => <div key={place.id} className="rounded-2xl bg-stone-100 px-4 py-3 dark:bg-stone-800"><p className="font-bold">{place.name}</p>{place.addressLabel && <p className="mt-0.5 truncate text-xs text-stone-500">{place.addressLabel}</p>}</div>)}</div> : <p className="mt-2 text-sm text-stone-500">Сохраните «Дом» или «Работу» при создании задачи — они появятся здесь.</p>}</section>

    {isAdmin && <section className="mt-4 rounded-3xl border border-violet-200 bg-violet-50 p-5 dark:border-violet-900 dark:bg-violet-950/40"><h2 className="flex items-center gap-2 font-black"><ShieldCheck className="size-5" /> Управление</h2><div className="mt-3 grid gap-2 sm:grid-cols-2"><Link href="/tasks" className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-stone-300 bg-white px-4 text-sm font-bold dark:border-stone-700 dark:bg-stone-900"><BriefcaseBusiness className="size-4" /> Все задачи</Link><Link href="/test-login" className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-stone-300 bg-white px-4 text-sm font-bold dark:border-stone-700 dark:bg-stone-900"><UsersRound className="size-4" /> Тестовый пользователь</Link></div></section>}
