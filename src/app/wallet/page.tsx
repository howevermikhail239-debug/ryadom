...
export default async function WalletPage() {
  ...
  return <main className="mx-auto min-h-dvh max-w-2xl px-4 pb-32 pt-5 sm:px-8">
    <header><Link href="/profile" className="inline-flex min-h-11 items-center gap-2 rounded-full bg-white px-4 text-sm font-bold shadow-sm dark:bg-stone-900"><ArrowLeft className="size-4" /> Профиль</Link><p className="mt-6 flex items-center gap-2 text-xs font-black uppercase tracking-wider text-emerald-700"><WalletCards className="size-4" /> Финансы</p><h1 className="mt-1 text-3xl font-black tracking-tight">Кошелёк</h1><p className="mt-1 text-sm text-stone-500">История финансовых операций по задачам. Безопасная сделка подключается отдельным этапом.</p></header>
